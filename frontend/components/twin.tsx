...
function TwinAvatar({
  ...
  const dims =
    size === 'lg' ? 'h-12 w-12' : size === 'md' ? 'h-11 w-11' : 'h-9 w-9';
  const textSize = size === 'lg' ? 'text-sm' : size === 'md' ? 'text-sm' : 'text-[11px]';

  if (!imageFailed) {
    return (
      <div
        className={`${dims} overflow-hidden rounded-2xl border border-white/10 bg-white/[0.06] shadow-[0_10px_24px_rgba(0,0,0,0.25)] ${className ?? ''}`}
      >
        <img
          src="/avatar.png"
          alt="Amit Kattyan"
  ...
  return (
    <div
      className={`flex ${dims} items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 via-teal-500 to-blue-600 ${textSize} font-semibold text-white shadow-[0_10px_24px_rgba(16,185,129,0.22)] ${className ?? ''}`}
    >
      AK
    </div>
  );
      ...
      <div className="shrink-0 flex items-center justify-between gap-3 border-b border-white/10 bg-white/[0.03] px-5 py-4">
        <div className="flex min-w-0 items-center gap-3">
          <div className="relative">
            <TwinAvatar size="md" className="shadow-[0_0_24px_rgba(16,185,129,0.25)]" />
              ...
              <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
                <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.28)]">
                  <div className="mb-6 flex items-center gap-3">
                    <TwinAvatar size="lg" className="shadow-[0_0_36px_rgba(16,185,129,0.22)]" />
                    <div>
                      <p className="text-base font-semibold tracking-tight text-white">Welcome</p>
                      <p className="text-xs text-slate-400">
                        High-signal conversations on architecture, AI, and platform strategy
                      </p>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-black/20 px-5 py-4">
                    <p className="text-sm leading-7 text-slate-100">
                      Welcome — I'm Amit's AI twin. Ask about AI platforms, AWS architecture, cloud
                      modernization, or large-scale system design.
                    </p>
                  </div>
                    ...
                    {[
                      {
                        ...
                        desc: 'Secure AWS foundations, multi-account design, resilience, and platform governance.',
                      },
                      {
                        icon: Cpu,
                        label: 'Agentic AI',
                        color: 'text-blue-300',
                        desc: 'Multi-agent systems, orchestration patterns, evaluation strategy, and enterprise adoption.',
                      },
                      {
                        icon: Waves,
                        label: 'Modernization',
                        color: 'text-cyan-300',
                        desc: 'Cloud migration, platform transformation, performance, cost efficiency, and operating models.',
                      },
                ...
                <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.24)]">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    ...
                    <div className="rounded-full border border-white/10 bg-black/20 px-2.5 py-1 text-[10px] uppercase tracking-[0.22em] text-slate-500">
                      Ready
                    </div>
                  </div>
                    ...
                    {SUGGESTED.map((q, index) => (
                      ...
                      >
                        <div className="mb-1.5 flex items-center justify-between gap-3">
                          ...
                          <span className="text-xs text-slate-500 transition-colors group-hover:text-emerald-200">
                            Use →
                          </span>
                        </div>
                  ...
                  <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
                    ...
                    <p className="text-xs leading-6 text-slate-500">
                      Ask specific architecture, AI, cloud, or transformation questions for sharper responses.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mx-auto flex w-full max-w-4xl flex-col gap-5">
              {messages.map((message) => (
                ...
                >
                  ...
                  <div
                    className={`max-w-[78%] rounded-[22px] px-4 py-3.5 shadow-[0_16px_40px_rgba(0,0,0,0.24)] ${
                      message.role === 'user'
                  ...
                  {message.role === 'user' && (
                    <div className="mb-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-[11px] font-semibold text-slate-300">
                      You
                    </div>
                  )}
                </div>
            ...
            <div className="rounded-[28px] border border-white/10 bg-black/20 p-2 backdrop-blur-xl">
              ...
              <div className="flex items-center justify-between gap-3 px-3 pb-1 pt-2">
                ...
                <p className="hidden text-[11px] text-slate-500 sm:block">
                  Focused on enterprise AI, AWS, modernization, and system design
                </p>
              </div>
            </div>
          </div>
